...
- `WEBHOOK_URL`은 선택 항목입니다. 로컬 데모나 문서 확인용 실행에서는 `NONE`이 안전하고, 실제 백엔드 연동 배포에서만 수신 endpoint를 넣으면 됩니다.
...
HTTP 엔드포인트와 요청/응답 예시는 [server/README.md](/c:/Users/user/Desktop/project/server/README.md:1)을 기준으로 봅니다.
...
`data/`는 Chroma 벡터 DB와 LangGraph 체크포인트 같은 재생성 가능한 런타임 산출물을 두는 폴더입니다.
